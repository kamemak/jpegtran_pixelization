...
#endif
  fprintf(stderr, "Switches for pixelization:\n");
  fprintf(stderr, "  -pixelize WxH+X+Y@N@M[a]   Can specify multiple times\n");
  exit(EXIT_FAILURE);
}
 ...
 */
{
  ...
  for (argn = 1; argn < argc; argn++) {
	...
	continue;
	  ...
	  ! jtransform_parse_pixelize_spec(pixelizeinfo+pixelizenum++, argv[argn])) {
	/* reject illegale requests */
	fprintf(stderr, "%s: bogus -pixelize argument '%s'\n",
		progname, argv[argn]);
	exit(EXIT_FAILURE);
      }
#endif
    ...
    } else if (keymatch(arg, "offset", 2)) {
      /* Select which extra markers to copy. */
      if (argn+4 >= argc)	/* advance to next argument */
	usage();
      if (( sscanf(argv[argn+1], "%d", coeff_offset+0) < 1) ||
          ( sscanf(argv[argn+2], "%d", coeff_offset+1) < 1) ||
          ( sscanf(argv[argn+3], "%d", coeff_offset+2) < 1) ||
          ( sscanf(argv[argn+4], "%d", coeff_offset+3) < 1) )
	usage();
      argn += 4;
      coeff_adj = 1;
