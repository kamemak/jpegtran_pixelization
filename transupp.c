 ...
 *  The pixelize function is handled in the same way as the wipe function.
 */
GLOBAL(boolean)
			      ...
			      jpeg_transform_info *info)
{
  ...
  if (info->crop) {
    ...
    if (info->transform == JXFORM_WIPE||info->transform == JXFORM_PIXELIZE) {
...
/* Added for ajpegtran
 *  Table indicates data width in bytes for 'datatype' in EXIF.
 */
...
/* Added for ajpegtran
 *  Clear EXIF IFD
 *  @param rm_thumb TRUE:for IFD1(thumbnail)  FALSE:for another IFD
 */
LOCAL(void)
clear_exif_parameters_IFD (JOCTET FAR * data, unsigned int length,
			unsigned int firstoffset, boolean is_motorola, boolean rm_thumb)
{
  unsigned int number_of_tags, tagnum, datatype, datanum, databyte, offset;
  unsigned int dataraw, jpeg_offset=0, jpeg_len=0, str_offset=0, str_len=0;
  ...
  data[firstoffset+0] = 0;
  data[firstoffset+1] = 0;
  firstoffset += 2;
  /* Scan all tags */
  for ( ; number_of_tags ; number_of_tags-- , firstoffset += 12 ) {
    ...
    /* Get data type */
    if (is_motorola) {
      datatype = GETJOCTET(data[firstoffset+3]);
    } else {
      datatype = GETJOCTET(data[firstoffset+2]);
    }
    /* Get data number */
    if (is_motorola) {
      datanum = GETJOCTET(data[firstoffset+6]);
      datanum <<= 8;
      datanum += GETJOCTET(data[firstoffset+7]);
    } else {
      datanum = GETJOCTET(data[firstoffset+5]);
      datanum <<= 8;
      datanum += GETJOCTET(data[firstoffset+4]);
    }
    /* Calculate data bytes */
    databyte = 0;
    if (datanum < sizeof(datawidth)/sizeof(datawidth[0])) {
      databyte = datawidth[datatype] * datanum;
    }
    if (datanum == 1 && rm_thumb) {
      /* Decode data */
      if (datatype == 3) {  /* Data type is SHORT */
        if (is_motorola) {
          dataraw = GETJOCTET(data[firstoffset+8]);
          dataraw <<= 8;
          dataraw += GETJOCTET(data[firstoffset+9]);
        } else {
          dataraw = GETJOCTET(data[firstoffset+9]);
          dataraw <<= 8;
          dataraw += GETJOCTET(data[firstoffset+8]);
      ...
      else if (datatype == 4) {  /* Data type is LONG */
        if (is_motorola) {
          dataraw = GETJOCTET(data[firstoffset+8]);
          dataraw <<= 8;
          dataraw |= GETJOCTET(data[firstoffset+9]);
          dataraw <<= 8;
          dataraw |= GETJOCTET(data[firstoffset+10]);
          dataraw <<= 8;
          dataraw |= GETJOCTET(data[firstoffset+11]);
        } else {
          dataraw = GETJOCTET(data[firstoffset+11]);
          dataraw <<= 8;
          dataraw |= GETJOCTET(data[firstoffset+10]);
      ...
      if (tagnum == 513 && datatype == 4) {
        /* 513 is JPEGInterchangeFormat tag.
         * It points the top of thumbnail JPEG. */
        jpeg_offset = dataraw;
      }
      else if (tagnum == 514 && datatype == 4) {
        /* 514 is JPEGInterchangeFormatLength tag.
         * It represents size of thumbnail JPEG. */
        jpeg_len = dataraw;
      }
      else if (tagnum == 273) {
        /* 273 is StripOffsets tag.
         * It points the top of thumbnail. */
        str_offset = dataraw;
      }
      else if (tagnum == 279) {
        /* 278 is StripByteCounts tag.
         * It represents size of thumbnail. */
        str_len = dataraw;
      }
    }

    /* If databyte is larger than 4. data is stored in the sparete area. */
    if(databyte > 4){
      /* Get offset */
      if (is_motorola) {
      ...
      /* Clear data */
      if(offset >= 6 && offset+databyte <= length){
        memset(data+offset,0,databyte);
      }
    }
    memset(data+firstoffset,0,12);
  }

  if(rm_thumb){
    /* Clear the thumbnail image body. */
    if(jpeg_offset >= 6 && jpeg_len && jpeg_offset+jpeg_len <= length){
       memset(data+jpeg_offset,0,jpeg_len);
    }
    if(str_offset >= 6 && str_len && str_offset+str_len <= length){
       memset(data+str_offset,0,str_len);
    }
  }
  
  if (firstoffset > length - 4) return; /* check end of data segment */
  /* Clear offset for next IFD (which may be 0) */
  data[firstoffset+0] = 0;
  data[firstoffset+1] = 0;
  data[firstoffset+2] = 0;
...
/* Added for ajpegtran
 *  Scan EXIF header,
 *  And remove orientation information, thumbnail and GEOTAG, if needed.
 */
LOCAL(void)
scan_exif_parameters_for_clear (JOCTET FAR * data, unsigned int length,
			boolean rm_orientation, boolean rm_thumbnail, boolean rm_geotag)
{
  ...
  firstoffset += 2;
  /* Search for Orientation in this IFD */
  for( ; number_of_tags ; number_of_tags-- , firstoffset += 12 ) {
    if (firstoffset > length - 12) return; /* check end of data segment */
    /* Get Tag number */
    if (is_motorola) {
    ...
    if (tagnum == 274 && rm_orientation) {
      /* 274 is 'Orientation' tag.
       * Clear the tag (Short x 1) */
      if (is_motorola) {
        data[firstoffset+8] = 0;
        data[firstoffset+9] = 1;
        data[firstoffset+10] = 0;
        data[firstoffset+11] = 0;
      } else {
        data[firstoffset+8] = 1;
        data[firstoffset+9] = 0;
        data[firstoffset+10] = 0;
        data[firstoffset+11] = 0;
      }
    }
    else if (tagnum == 0x8825 && rm_geotag) {
      /* 0x8825 is 'GPSInfoIFD'(offset value) */
      if (is_motorola) {
        offset = GETJOCTET(data[firstoffset+8]);
        offset <<= 8;
        offset |= GETJOCTET(data[firstoffset+9]);
      ...
      // Call sub function for clear the IFD.
      if(offset >= 6){
        clear_exif_parameters_IFD(data,length,offset,is_motorola,FALSE);
      }
    }
  }
  if (firstoffset > length - 4) return; /* check end of data segment */
  if (rm_thumbnail) {
    /* At the last of IFD0, offset for IFD1 which contain thumbnail is exist.
     * Clear it, if option specified. */
    unsigned int ifd1offset;
    if (is_motorola) {
      ifd1offset = GETJOCTET(data[firstoffset+0]);
      ifd1offset <<= 8;
      ifd1offset |= GETJOCTET(data[firstoffset+1]);
      ifd1offset <<= 8;
      ifd1offset |= GETJOCTET(data[firstoffset+2]);
      ifd1offset <<= 8;
      ifd1offset |= GETJOCTET(data[firstoffset+3]);
    } else {
      ifd1offset = GETJOCTET(data[firstoffset+3]);
      ifd1offset <<= 8;
      ifd1offset |= GETJOCTET(data[firstoffset+2]);
    ...
    // Call sub function for clear the IFD1.
    if(ifd1offset >= 6){
      clear_exif_parameters_IFD(data,length,ifd1offset,is_motorola,TRUE);
      data[firstoffset+0] = 0;
      data[firstoffset+1] = 0;
      data[firstoffset+2] = 0;
      data[firstoffset+3] = 0;
			      ...
			      jpeg_transform_info *info)
{
  ...
  /* Added for ajpegtran
   *  If remove header options are specifed and EXIF header exist,
   *  call scanning EXIF header function.
   */
  jpeg_saved_marker_ptr marker;
  for (marker = srcinfo->marker_list; marker != NULL; marker = marker->next) {
    if ( marker->marker == JPEG_APP0+1 &&     // maker is EXIF (JPEG_APP1)
      marker->data_length >= 6 &&             // lenght is enoght
      GETJOCTET(marker->data[0]) == 0x45 &&   // Magic number 'Exif\0\0'
      GETJOCTET(marker->data[1]) == 0x78 &&
      GETJOCTET(marker->data[2]) == 0x69 &&
      GETJOCTET(marker->data[3]) == 0x66 &&
      GETJOCTET(marker->data[4]) == 0 &&
      GETJOCTET(marker->data[5]) == 0 &&
      (dstinfo->remove_orientation_info||dstinfo->remove_thumbnail||dstinfo->remove_geotag)) {  // any option is specifed?
      scan_exif_parameters_for_clear(marker->data + 6,
        marker->data_length - 6,
        dstinfo->remove_orientation_info, 
        dstinfo->remove_thumbnail, 
        dstinfo->remove_geotag);
    }
  }
